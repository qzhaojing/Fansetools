...
def init_config():
    """初始化配置目录和文件"""
    config_dir = Path("c:\\FANse3g")
    
    try:
        config_dir.mkdir(parents=True, exist_ok=True)
        print(f"配置目录已准备就绪: {config_dir}")
        
        # 初始化配置文件（如果不存在）
        if not (config_dir / "fanse3.ini").exists():
            with open(config_dir / "fanse3.ini", "w") as f:
                f.write("[DEFAULT]\n")
            print("配置文件已初始化")
        ...
        print(f"初始化失败: {str(e)}")
        # 可以尝试使用备用目录
        backup_dir = Path.home() / ".fanse3"
        backup_dir.mkdir(parents=True, exist_ok=True)
        print(f"使用备用目录: {backup_dir}")
        return backup_dir
...
class FanseRunner:
    ...
    #     if path.is_dir():
    #         found = None
    #         for exe in ['fanse3', 'fanse3g', 'fanse']:
    #             exe_path = path / (exe + ('.exe' if os.name == 'nt' else ''))
    #             if exe_path.exists():
    #                 found = exe_path
    #                 break
    ...
    #     ConfigManager.save_config('fanse3exe', path.name)
    #     logger.info(f"FANSe路径已配置: {path}")
